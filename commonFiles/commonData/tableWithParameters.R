...
path="/home/juanpc/Documents/SFU_files/Research/multiMeasurementCodes/commonFiles/"
source(paste(path,'jarCoordinates.R',sep=""));
source(paste(path,'locateJars.R',sep=""));source(paste(path,'depositionEstimation.R',sep=""))
...
tableWithParameters=function(case){
	...
	if(case==2){
		...
		parameters=read.table(paste(path,'commonData/','designScaled64.txt',sep=""))
	}
	...
	sources=depositionEstimation(jar.location,n,nx,ny,'./',1,'s')
