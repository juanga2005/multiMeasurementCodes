...
#Last Modified: mié 09 ago 2017 23:14:08 PDT
...
Aq=function(Xtest,qtest,q,ind){
	...
	dA=0.026
	temp=sapply(1:dim(a)[3],function(i) dA*a[,,i]%*%qtest[i,])
	l=list(temp,a)
...
ex=function(aq,l,ind,type){
	...
	if(type==2){
		aux2=sapply(1:dim(aq[[2]])[3],function(i)((0.026*aq[[1]]-m)[,i]%*%t(t(aq[[2]][,ind,i]))))
		return(aux2*exp(aux))
	}
	if(type==3){
		aux3=Norm(0.026*aq[[1]]-m)^2
		return(aux3*exp(aux))
	}
}
